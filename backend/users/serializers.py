...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_courseIds(self, obj):
        ...
        if obj.role == 'lecturer':
            return list(Program.objects.filter(lecturer=obj).values_list('id', flat=True))
        # Department heads can access all courses
        return list(Program.objects.all().values_list('id', flat=True))
    
    class Meta:
        model = User
        fields = ['id', 'username', 'email', 'first_name', 'last_name', 'role', 'department', 'university', 'is_active', 'is_staff', 'date_joined', 'courseIds']

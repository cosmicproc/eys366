...
export default function MainGraph() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (!targetCourseId && user?.role === "lecturer") {
          console.warn("No course selected and user has no courses assigned");
          setLoading(false);
          return;
        }
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      const detail = (e as CustomEvent).detail as
        | {
            id: number;
            name: string;
            type: "course_content" | "course_outcome" | "program_outcome";
          }
        | undefined;
      if (!detail) return;
      setNodes((prev) => {
        const y =
          80 + prev.filter((n) => n.id.startsWith(detail.type)).length * 100;
        const xMap = {
          course_content: 50,
          course_outcome: 400,
          program_outcome: 750,
        ...
        const targetPosition =
          detail.type === "course_content" ? Position.Right : Position.Left;
        const newNode: Node = {
    ...
    return () => {
      window.removeEventListener("giraph:new-node", handler as EventListener);
    };
  }, []);
        ...
        if (change.type === "remove") {
          ...
          if (edge?.data?.relationId) {
            ...
            deleteRelation(relationId)
              ...
              .catch((error) => {
                console.error(`Failed to delete relation: ${error.message}`);
  ...
  const submitWeight = async () => {
    ...
    try {
      ...
      if (modalMode === "create" && pendingConnection) {
        ...
        const response = await createRelation(node1_id, node2_id, weightValue);
            ...
            prev.map((e) =>
              e.id === editingEdge.id
                ? {
                    ...
                    data: {
                      ...(e.data || {}),
                      weight: (editingEdge.data as { weight?: number })?.weight,
                    },
                  }
                : e
            )
    ...
    >
      {loading ? (
        ...
        <ReactFlow
          ...
          nodeTypes={createNodeTypes(handleNodeEdit, handleNodeDelete)}
          onConnect={onConnect}
      ...
      <Modal
        ...
        onClose={closeModal}
        title={modalMode === "create" ? "Set edge weight" : "Edit edge weight"}
        centered
      >
